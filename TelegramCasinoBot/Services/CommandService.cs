...
*Новые возможности:*
• ??? **Исследуйте локации** - перемещайтесь по сетке 10x10
...
• '??? Карта' - карта текущей локации
• '??? Карта мира' - общая карта
*Удачи в исследовании Аркадии!*";
        ...
        private async Task HandleAttackCommand(long chatId, Player player)
        {
            ...
            if (hasEnemy)
                await _botClient.SendTextMessageAsync(chatId, "?? Вы готовы к бою! Система сражений в разработке...");
            else
                await _botClient.SendTextMessageAsync(chatId, "?? Здесь нет врагов для атаки.");
        }
