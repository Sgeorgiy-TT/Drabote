...
namespace TelegramMetroidvaniaBot.Services
{
    public class CharacterIconService
    {
        ...
        // Модель для хранения состояния выбора иконки
        ...
        private async Task<List<string>> GetAvailableIcons(string gender, string race)
        {
            ...
            // Маппинг русских названий на папки
            ...
            {
                // Ищем файлы, которые начинаются с префикса пола
            ...
            // Если не нашли иконок для комбинации, используем дефолтные
            if (!icons.Any())
            {
        ...
        private async Task<List<string>> GetDefaultIcons(string gender)
        {
            ...
            // Ищем в любой папке иконки с нужным префиксом пола
            ...
            return defaultIcons.Take(10).ToList(); // Ограничиваем количество дефолтных иконок
        }
        ...
        {
            // Этот метод будет вызываться после подтверждения выбора
            // Пока возвращаем первую иконку как пример
