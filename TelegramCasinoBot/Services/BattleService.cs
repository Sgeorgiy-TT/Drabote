...
?? Ваше здоровье: {Math.Max(0, player.Health)}/{player.MaxHealth}
...
?? Новое умение: Сила Древних";
        ...
        private static InlineKeyboardMarkup GetBattleKeyboard()
        {
            return new InlineKeyboardMarkup(new[]
            {
                new[]
                {
                    InlineKeyboardButton.WithCallbackData("?? Атака", "attack_boss"),
                    InlineKeyboardButton.WithCallbackData("??? Защита", "defend_boss")
                },
                new[]
                {
                    InlineKeyboardButton.WithCallbackData("?? Способность", "ability_boss"),
                    InlineKeyboardButton.WithCallbackData("???>? Бегство", "flee_boss")
                }
            });
        }
    }
